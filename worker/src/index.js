...
function repairUtf8Mojibake(value){let s=String(value??'');const bad=/[ÃÃÃÃÃ¢â¬â¢Ã¢â¬ÅÃ¢â¬]/;const cp1252={0x20ac:0x80,0x201a:0x82,0x192:0x83,0x201e:0x84,0x2026:0x85,0x2020:0x86,0x2021:0x87,0x2c6:0x88,0x2030:0x89,0x160:0x8a,0x2039:0x8b,0x152:0x8c,0x17d:0x8e,0x17e:0x9e,0x2018:0x91,0x2019:0x92,0x201c:0x93,0x201d:0x94,0x2022:0x95,0x2013:0x96,0x2014:0x97,0x2dc:0x98,0x2122:0x99,0x161:0x9a,0x203a:0x9b,0x153:0x9c,0x178:0x9f,0x164:0x8d};for(let pass=0;pass<3&&bad.test(s);pass++){try{const bytes=Uint8Array.from([...s].map(ch=>{const cp=ch.codePointAt(0);return cp1252[cp]??(cp<=255?cp:0)}));const fixed=new TextDecoder('utf-8',{fatal:true}).decode(bytes);if(fixed===s)break;s=fixed}catch{break}}return s}
...
async function loadWhatsappStory(){const st=document.getElementById('whatsappStoryStatus'),list=document.getElementById('whatsappStoryList');if(!st||!list)return;st.textContent=safeFa('Ø¯Ø± Ø­Ø§Ù Ø¯Ø±ÛØ§ÙØª Ø±Ø³Ø§ÙÙâÙØ§â¦');try{const d=await api('/api/telegram/media?limit=12');const a=d.items||[];st.className='status ok';st.textContent=safeFa(a.length+' Ø±Ø³Ø§ÙÙ Ø¢ÙØ§Ø¯Ù Ø§Ø³Øª');list.innerHTML=a.length?a.map(x=>'<div class="item"><div class="itemhead"><b>ð± '+esc(safeFa(x.media_type))+'</b><span class="pill">'+esc(safeFa(x.created_at||''))+'</span></div><div class="mini">'+esc(safeFa(x.caption||'Ø¨Ø¯ÙÙ Ú©Ù¾Ø´Ù'))+'</div><div class="actions" style="margin-top:10px"><button class="btn primary" onclick="shareTelegramMedia(&quot;'+esc(x.id)+'&quot;)">Ø§Ø±Ø³Ø§Ù Ø¨Ù WhatsApp / Ø¯Ø§ÙÙÙØ¯</button></div></div>').join(''):'<div class="empty">'+safeFa('ÙÙÙØ² Ø±Ø³Ø§ÙÙâØ§Û Ø§Ø² Ú©Ø§ÙØ§Ù Ø¯Ø±ÛØ§ÙØª ÙØ´Ø¯Ù Ø§Ø³Øª.')+'</div>'}catch(e){st.className='status error';st.textContent=safeFa('Ø®Ø·Ø§: '+e.message)}}
async function loadTelegramMedia(){const st=document.getElementById('telegramMediaStatus'),list=document.getElementById('telegramMediaList');if(!st||!list)return;st.textContent=safeFa('Ø¯Ø± Ø­Ø§Ù Ø¯Ø±ÛØ§ÙØª Ø±Ø³Ø§ÙÙâÙØ§Û Ø¬Ø¯ÛØ¯ Ú©Ø§ÙØ§Ùâ¦');try{const d=await api('/api/telegram/media?limit=12');const a=d.items||[];st.className='status ok';st.textContent=safeFa(a.length+' Ø±Ø³Ø§ÙÙ Ø¯Ø±ÛØ§ÙØª Ø´Ø¯');list.innerHTML=a.length?a.map(x=>'<div class="item"><div class="itemhead"><b>'+esc(safeFa(x.media_type))+'</b><span class="pill">'+esc(safeFa(x.created_at||''))+'</span></div><div class="mini">'+esc(safeFa(x.caption||'Ø¨Ø¯ÙÙ Ú©Ù¾Ø´Ù'))+'</div><div class="actions" style="margin-top:10px"><button class="btn secondary" onclick="downloadTelegramMedia(&quot;'+esc(x.id)+'&quot;)">ÙØ´Ø§ÙØ¯Ù / Ø¯Ø±ÛØ§ÙØª</button></div></div>').join(''):'<div class="empty">'+safeFa('ÙÙÙØ² Ø±Ø³Ø§ÙÙ Ø¬Ø¯ÛØ¯Û Ø§Ø² Ú©Ø§ÙØ§Ù Ø¯Ø±ÛØ§ÙØª ÙØ´Ø¯Ù. Ø¨Ø¹Ø¯ Ø§Ø² ÙØ¹Ø§ÙâØ³Ø§Ø²ÛØ Ù¾Ø³ØªâÙØ§Û Ø¬Ø¯ÛØ¯ Ú©Ø§ÙØ§Ù Ø§ÛÙØ¬Ø§ ÙÛâØ¢ÛÙØ¯.')+'</div>'}catch(e){st.className='status error';st.textContent=safeFa('Ø®Ø·Ø§: '+e.message)}}
async function setupTelegramMedia(){const st=document.getElementById('telegramMediaStatus');st.className='status';st.textContent=safeFa('Ø¯Ø± Ø­Ø§Ù ÙØ¹Ø§ÙâØ³Ø§Ø²Û Ø¯Ø±ÛØ§ÙØª Ø±Ø³Ø§ÙÙâ¦');try{await api('/api/telegram/webhook/setup',{method:'POST'});st.className='status ok';st.textContent=safeFa('Ø¯Ø±ÛØ§ÙØª Ø±Ø³Ø§ÙÙ ÙØ¹Ø§Ù Ø´Ø¯. Ø­Ø§ÙØ§ Ù¾Ø³ØªâÙØ§Û Ø¬Ø¯ÛØ¯ Ú©Ø§ÙØ§Ù ÙØ§Ø±Ø¯ Ù¾ÙÙ ÙÛâØ´ÙÙØ¯.');await loadTelegramMedia()}catch(e){st.className='status error';st.textContent=safeFa('Ø®Ø·Ø§: '+e.message)}}
...
async function loadAdOverview(){const b=document.getElementById('adOverviewBody');if(!b)return;try{const d=await api('/api/ads/overview'),m=d.metrics||{};b.innerHTML='<div class="two"><div><div class="stat">'+esc(m.found_today||0)+'</div><div class="muted">ÙØ¹Ø§ÙÛØª Ø§ÙØ±ÙØ²</div></div><div><div class="stat">'+esc(m.hot||0)+'</div><div class="muted">ÙÛØ¯ Ø¯Ø§Øº</div></div><div><div class="stat">'+esc(m.negotiation||0)+'</div><div class="muted">ÙØ°Ø§Ú©Ø±Ù</div></div><div><div class="stat">'+esc(m.due_followups||0)+'</div><div class="muted">Ù¾ÛÚ¯ÛØ±Û Ø³Ø±Ø±Ø³ÛØ¯</div></div></div><div class="mini" style="margin-top:10px">'+esc(m.ad_opportunities||0)+' ÙØ±ØµØª ØªØ¨ÙÛØºØ§ØªÛ Â· '+esc(m.customers||0)+' ÙØ´ØªØ±Û Â· '+esc(m.ad_targets||0)+' ÙØ¯Ù ØªØ¨ÙÛØºØ§ØªÛ</div>';b.className='status ok';const list=document.getElementById('adOpportunityList');if(list)list.innerHTML=(d.items||[]).map(x=>'<div class="item"><div class="itemhead"><b>'+esc(safeFa(x.name||'Ø¨Ø¯ÙÙ ÙØ§Ù'))+'</b><span class="pill">Score '+esc(x.score||0)+'</span></div><div class="mini">'+esc(safeFa(x.type||''))+' Â· '+esc(safeFa(x.stage||''))+'</div>'+(x.ad_opportunity?'<div class="mini">ÙØ±ØµØª ØªØ¨ÙÛØºØ§ØªÛ Ø´ÙØ§Ø³Ø§ÛÛ Ø´Ø¯</div>':'')+(x.contact_url?'<div class="actions" style="margin-top:8px"><button class="btn secondary" onclick="window.open(&quot;'+esc(safeFa(x.contact_url))+'&quot;,&quot;_blank&quot;)">ÙØ³ÛØ± ØªÙØ§Ø³</button></div>':'')+'</div>').join('')||'<div class="empty">ÙÙÙØ² ÙØ±ØµØª ØªØ¨ÙÛØºØ§ØªÛ Ø«Ø¨Øª ÙØ´Ø¯Ù.</div>'}catch(e){b.className='status error';b.textContent='Ø®Ø·Ø§: '+e.message}}
async function loadAdActionCenter(){const b=document.getElementById('adActionCenter');if(!b)return;b.textContent='Ø¯Ø± Ø­Ø§Ù Ø¢ÙØ§Ø¯ÙâØ³Ø§Ø²Û Ø§ÙØ¯Ø§ÙâÙØ§â¦';try{const d=await api('/api/ads/action-center'),c=d.counts||{};const labels={today:'Ù¾ÛÚ¯ÛØ±Û Ø§ÙØ±ÙØ²',urgent:'Ø§ÙØ¯Ø§Ù ÙÙØ±Û',opportunity:'ÙØ±ØµØª ØªØ¨ÙÛØº'};b.innerHTML='<div class="two">'+Object.entries(labels).map(([k,v])=>'<div><div class="stat">'+esc(c[k]||0)+'</div><div class="muted">'+v+'</div></div>').join('')+'</div>'+(d.items||[]).map(x=>'<div class="item"><div class="itemhead"><b>'+esc(safeFa(x.name||'Ø¨Ø¯ÙÙ ÙØ§Ù'))+'</b><span class="pill">'+esc(String(x.score||0))+'</span></div><div class="mini">'+esc(labels[x.bucket]||'Ø¨Ø±Ø±Ø³Û')+' Â· '+esc(safeFa(x.stage||''))+'</div><div class="mini">'+esc(safeFa(x.action||''))+'</div><div class="mini">'+esc(safeFa((x.factors||[]).join(' Â· ')))+'</div></div>').join('')||'<div class="empty">ÙØ¹ÙØ§Ù Ø§ÙØ¯Ø§Ù ÙØ´Ø®ØµÛ ÙØ¬ÙØ¯ ÙØ¯Ø§Ø±Ø¯.</div>';b.className='status ok'}catch(e){b.className='status error';b.textContent='Ø®Ø·Ø§: '+e.message}}

async function loadAdIntelligence(){const b=document.getElementById('adIntelligenceBody');if(!b)return;try{const d=await api('/api/ads/intelligence'),m=d.metrics||{};b.innerHTML='<div class="two"><div><div class="stat">'+esc(m.hot||0)+'</div><div class="muted">ÙÛØ¯ÙØ§Û Ø¯Ø§Øº</div></div><div><div class="stat">'+esc(m.opportunities||0)+'</div><div class="muted">ÙØ±ØµØª ØªØ¨ÙÛØº</div></div><div><div class="stat">'+esc(m.action_items||0)+'</div><div class="muted">Ø§ÙØ¯Ø§Ù Ø¨Ø¹Ø¯Û</div></div><div><div class="stat">'+esc(m.duplicate_domains||0)+'</div><div class="muted">Ø¯Ø§ÙÛÙ ØªÚ©Ø±Ø§Ø±Û</div></div></div><div class="mini" style="margin-top:10px">Ú©Ù Lead: '+esc(m.total||0)+' Â· ÙØ¹Ø§Ù: '+esc(m.active||0)+'</div>';const list=document.getElementById('adActionList');if(list)list.innerHTML=(d.action_items||[]).slice(0,10).map(x=>'<div class="item"><div class="itemhead"><b>'+esc(safeFa(x.name||'Ø¨Ø¯ÙÙ ÙØ§Ù'))+'</b><span class="pill">'+esc(x.score||0)+'</span></div><div class="mini">'+esc(safeFa(x.type||''))+' Â· '+esc(safeFa(x.stage||''))+'</div><div class="mini">'+esc(safeFa(x.action||''))+'</div><div class="mini">'+esc(safeFa((x.factors||[]).join(' Â· ')))+'</div></div>').join('')||'<div class="empty">Ø§ÙØ¯Ø§Ù ÙÙØ±Û Ø«Ø¨Øª ÙØ´Ø¯Ù.</div>';b.className='status ok'}catch(e){b.className='status error';b.textContent='Ø®Ø·Ø§: '+e.message}}
