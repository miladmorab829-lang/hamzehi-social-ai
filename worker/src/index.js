...
function repairUtf8Mojibake(value){let s=String(value??'');const cp1252={0x20ac:0x80,0x201a:0x82,0x192:0x83,0x201e:0x84,0x2026:0x85,0x2020:0x86,0x2021:0x87,0x2c6:0x88,0x2030:0x89,0x160:0x8a,0x2039:0x8b,0x152:0x8c,0x17d:0x8e,0x17e:0x9e,0x2018:0x91,0x2019:0x92,0x201c:0x93,0x201d:0x94,0x2022:0x95,0x2013:0x96,0x2014:0x97,0x2dc:0x98,0x2122:0x99,0x161:0x9a,0x203a:0x9b,0x153:0x9c,0x178:0x9f,0x164:0x8d};const bad=/[ÃÃÃÃÃÃÃÃÃÃÃ Ã¡Ã¢Ã£Ã¤Ã¥Ã¦Ã§Ã¨Ã©ÃªÃ«Ã¬Ã­Ã®Ã¯Ã°Ã±Ã²Ã³Ã´ÃµÃ¶Ã·Ã¸Ã¹ÃºÃ»Ã¼Ã½Ã¾Ã¿â¬âÆââ¦â â¡Ëâ°Å â¹ÅÅ½âââââ¢ââËâ¢Å¡âºÅÅ¾Å¸]/;if(!bad.test(s))return s;return s.split(/(\t|\n|\r|\s+)/).map(part=>{if(!part||!bad.test(part))return part;let p=part;for(let pass=0;pass<4;pass++){if(!bad.test(p))break;try{const bytes=[];for(const ch of p){const cp=ch.codePointAt(0);if(cp1252[cp]!==undefined)bytes.push(cp1252[cp]);else if(cp<=0xff)bytes.push(cp);else throw Error('non-byte')}const decoded=new TextDecoder('utf-8',{fatal:true}).decode(Uint8Array.from(bytes));if(decoded===p)break;p=decoded}catch{break}}return p}).join('')}
function safeFa(value){return repairUtf8Mojibake(value)}
function sectionFa(value){let s=String(value??'');return s.split(/(\s+)/).map(part=>{if(/^\s+$/.test(part))return part;let p=part;for(let n=0;n<3;n++){const r=repairUtf8Mojibake(p);if(r===p)break;p=r}return p}).join('')}
...
async function runAdFollowups(){const b=document.getElementById('adFollowupStatus');if(b){b.className='status';b.textContent='Ø¯Ø± Ø­Ø§Ù Ø¢ÙØ§Ø¯ÙâØ³Ø§Ø²Û Ù¾ÛÚ¯ÛØ±ÛâÙØ§Û Ø³Ø±Ø±Ø³ÛØ¯â¦'}try{const d=await api('/api/ads/followups/run',{method:'POST',body:'{}'});if(b){b.className='status ok';b.textContent=esc(sectionFa(d.prepared||0))+' Ù¾ÛÚ¯ÛØ±Û Ø¢ÙØ§Ø¯Ù Ø´Ø¯ Â· '+esc(sectionFa(d.skipped||0))+' ÙÙØ±Ø¯ Ø±Ø¯ Ø´Ø¯.'}await loadAdOverview();await loadAdAutopilotStatus()}catch(e){if(b){b.className='status error';b.textContent='Ø®Ø·Ø§: '+e.message}}}

async function runAdAutopilot(){const st=document.getElementById('adsStatus'),list=document.getElementById('adsList');if(!st||!list)return;const city=document.getElementById('adsCity').value.trim(),type=document.getElementById('adsType').value,extra=document.getElementById('adsExtra').value.trim();st.className='status';st.textContent='Ø®ÙØ¨Ø§Ù Ø®ÙØ¯Ú©Ø§Ø± Ø¯Ø± Ø­Ø§Ù Ø¨Ø±Ø±Ø³Û Ø¢ÙØ§Ø¯Ú¯Û Ù Ø³Ù¾Ø³ Ø§Ø¬Ø±Ø§Û Ø¹ÙÙÛØ§Øª Ø§Ø³Øªâ¦';list.innerHTML='';try{const pf=await api('/api/ads/preflight');const failed=(pf.checks||[]).filter(x=>x.status==='FAIL');if(failed.length){throw Error('Preflight ÙØ§ÙÙÙÙ Ø§Ø³Øª: '+failed.map(x=>x.name+': '+(x.details||x.value||'FAIL')).join(' Â· '))}if(pf.safe!==true){throw Error('Preflight Ø§Ø¬Ø§Ø²Ù Ø§Ø¬Ø±Ø§Û Autopilot Ø±Ø§ ØµØ§Ø¯Ø± ÙÚ©Ø±Ø¯.')}const d=await api('/api/ads/autopilot',{method:'POST',body:JSON.stringify({city,type,extra})});const m=d.summary||{};st.className='status ok';st.textContent='Ø®ÙØ¨Ø§Ù Ø®ÙØ¯Ú©Ø§Ø± ØªÙØ§Ù Ø´Ø¯ Â· '+(m.found||0)+' Ù¾ÛØ¯Ø§ Ø´Ø¯ Â· '+(m.new_leads||0)+' Ø¬Ø¯ÛØ¯ Â· '+(m.drafted||0)+' ÙØ°Ø§Ú©Ø±Ù Ø¢ÙØ§Ø¯Ù Ø´Ø¯';list.innerHTML=(d.items||[]).slice(0,20).map(x=>'<div class="item"><div class="itemhead"><b>'+esc(sectionFa(x.name||'Ø¨Ø¯ÙÙ ÙØ§Ù'))+'</b><span class="pill">'+esc(x.score||0)+'</span></div><div class="mini">'+esc(sectionFa(x.type||''))+' Â· '+esc(sectionFa(x.url||''))+'</div>'+(x.contact_url?'<div class="mini">Ø±Ø§Ù ØªÙØ§Ø³: '+esc(sectionFa(x.contact_url))+'</div>':'')+(x.draft?'<div class="text" style="margin-top:6px">'+esc(sectionFa(x.draft))+'</div>':'')+'</div>').join('')||'<div class="empty">ÙÙØ±Ø¯ Ø¬Ø¯ÛØ¯Û Ù¾ÛØ¯Ø§ ÙØ´Ø¯.</div>';await loadAdTargets();await loadAdAutopilotStatus()}catch(e){st.className='status error';st.textContent='Ø®Ø·Ø§: '+e.message}}
async function loadAdPreflight(){const b=document.getElementById('adPreflight');if(!b)return;b.className='status';b.textContent='Ø¯Ø± Ø­Ø§Ù Ø¨Ø±Ø±Ø³Û Ø¢ÙØ§Ø¯Ú¯Û Ø³ÛØ³ØªÙâ¦';try{const d=await api('/api/ads/preflight');const bad=(d.checks||[]).filter(x=>x.status==='FAIL');b.innerHTML=(d.checks||[]).map(x=>{const cls=x.status==='PASS'?'ok':x.status==='READY'?'warn':x.status==='SKIP'?'warn':'off';const label=x.status==='PASS'?'Ø¢ÙØ§Ø¯Ù':x.status==='READY'?'Ø¢ÙØ§Ø¯Ù Ø¨Ø±Ø§Û Ø¨Ø±Ø±Ø³Û':x.status==='SKIP'?'Ø±Ø¯ Ø´Ø¯Ù':'Ø®Ø·Ø§';return '<div class="mini"><span class="status-dot '+cls+'"></span>'+label+' Â· '+esc(sectionFa(x.name))+': '+esc(sectionFa(x.details||x.value||x.status))+'</div>'}).join('');b.className=bad.length?'status error':'status ok'}catch(e){b.className='status error';b.textContent='Ø®Ø·Ø§: '+e.message}}
async function loadAdAutopilotStatus(){const b=document.getElementById('adAutopilotStatus');if(!b)return;try{const d=await api('/api/ads/autopilot/status'),m=d.metrics||{};b.innerHTML='<div class="stat"><span class="status-dot '+(d.enabled?'ok':'off')+'"></span>'+(d.enabled?'Ø±ÙØ´Ù':'Ø®Ø§ÙÙØ´')+'</div><div class="mini">'+(d.enabled?'Ø§Ø¬Ø±Ø§Û Ø®ÙØ¯Ú©Ø§Ø± ÙØ± Û±Ûµ Ø¯ÙÛÙÙ ÙØ¹Ø§Ù Ø§Ø³Øª':'Ø§Ø¬Ø±Ø§Û Ø®ÙØ¯Ú©Ø§Ø± ÙØªÙÙÙ Ø§Ø³Øª')+' Â· '+esc(sectionFa(m.ad_targets||0))+' ÙØ¯Ù ØªØ¨ÙÛØºØ§ØªÛ Â· '+esc(sectionFa(m.hot||0))+' Ø¯Ø§Øº Â· '+esc(sectionFa(m.negotiation||0))+' ÙØ°Ø§Ú©Ø±Ù Â· '+esc(sectionFa(m.followups||0))+' Ù¾ÛÚ¯ÛØ±Û Â· '+esc(sectionFa(m.customers||0))+' ÙØ´ØªØ±Û</div>';b.className=d.enabled?'status ok':'status'}catch(e){b.className='status error';b.textContent='Ø®Ø·Ø§: '+e.message}}async function startAdAutopilot{const type=document.getElementById('adsType')?.value||'all',city=document.getElementById('adsCity')?.value.trim()||'',extra=document.getElementById('adsExtra')?.value.trim()||'',b=document.getElementById('adAutopilotControlStatus');if(b){b.className='status';b.textContent='Ø¯Ø± Ø­Ø§Ù Ø±ÙØ´ÙâÚ©Ø±Ø¯Ù Ø®ÙØ¨Ø§Ù Ø®ÙØ¯Ú©Ø§Ø±â¦'}try{const d=await api('/api/ads/autopilot/start',{method:'POST',body:JSON.stringify({type,city,extra})});if(b){b.className=d.enabled?'status ok':'status error';b.textContent=d.enabled?'ð¢ AUTOPILOT Ø±ÙØ´Ù Ø´Ø¯ Ù ØªØ§ STOP Ø§Ø¯Ø§ÙÙ ÙÛâØ¯ÙØ¯.':'ð´ AUTOPILOT Ø±ÙØ´Ù ÙØ´Ø¯.'}await loadAdAutopilotStatus();await loadAdOverview()}catch(e){if(b){b.className='status error';b.textContent='Ø®Ø·Ø§: '+e.message}}}
async function stopAdAutopilot(){const b=document.getElementById('adAutopilotControlStatus');if(b){b.className='status';b.textContent='Ø¯Ø± Ø­Ø§Ù Ø®Ø§ÙÙØ´âÚ©Ø±Ø¯Ù Ø®ÙØ¨Ø§Ù Ø®ÙØ¯Ú©Ø§Ø±â¦'}try{await api('/api/ads/autopilot/stop',{method:'POST',body:'{}'});if(b){b.className='status ok';b.textContent='AUTOPILOT Ø®Ø§ÙÙØ´ Ø´Ø¯. Ø§Ø¬Ø±Ø§Û Ø²ÙØ§ÙâØ¨ÙØ¯ÛâØ´Ø¯Ù ÙØªÙÙÙ Ø§Ø³Øª.'}await loadAdAutopilotStatus()}catch(e){if(b){b.className='status error';b.textContent='Ø®Ø·Ø§: '+e.message}}}
