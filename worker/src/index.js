...
async function publicTelegramMediaProxy(env,req){
  ...
  const path=String(fd.result.file_path||'').toLowerCase(); let contentType='application/octet-stream'; if(path.endsWith('.jpg')||path.endsWith('.jpeg')) contentType='image/jpeg'; else if(path.endsWith('.png')) contentType='image/png'; else if(path.endsWith('.webp')) contentType='image/webp'; else if(path.endsWith('.mp4')) contentType='video/mp4'; const h=new Headers(); h.set('Content-Type',contentType); h.set('Cache-Control','public, max-age=300');
